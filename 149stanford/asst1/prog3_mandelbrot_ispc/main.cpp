...
void usage(const char* progname) {
    ...
    printf("  -s  --scale <scaleValue>,<shiftX>,<shiftY>   Manually adjust scale\n");
...
int main(int argc, char** argv) {
    ...
    while ((opt = getopt_long(argc, argv, "t::v:s:?", long_options, NULL)) != EOF) {
        ...
        {
            useTasks = true;
            taskCount = optarg != NULL ? atoi(optarg) : DEFAULT_TASK;
            break;
        }
        case 'v':
        {
        ...
        case 's':
        {
            ...
            float shiftY = .0f;
            char *optptr;
            optptr = strtok(optarg, ",");
            if (optptr == NULL) {
                continue;
            }
            scaleValue = atof(optptr);
            optptr = strtok(NULL, ",");
            if (optptr == NULL) {
                continue;
            }
    ...
    if (useTasks) {
        ...
        //
        printf("--- Tasking: using %zu tasks ---\n", taskCount);
        for (int i = 0; i < 3; ++i) {
            double startTime = CycleTimer::currentSeconds();
            mandelbrot_ispc_withtasks(x0, y0, x1, y1, width, height, maxIterations, output_ispc_tasks, taskCount);
            double endTime = CycleTimer::currentSeconds();
